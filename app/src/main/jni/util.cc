 ...
 *
 * Implementacao das utilidades (derivada do sample oficial do Cardboard SDK).
 */

#include "util.h"

#include <android/log.h>
#include <string.h>  // strtok_r e strstr
#include <time.h>
#include <unistd.h>
...
bool LoadPngFromAssetManager(JNIEnv* env, jobject java_asset_mgr, int target,
  ...
  jclass asset_manager_class = env->FindClass("android/content/res/AssetManager");
  ...
  jobject image_stream = env->CallObjectMethod(java_asset_mgr, open_method, j_path);
  jobject image_obj = env->CallStaticObjectMethod(bitmap_factory_class,
                                                  decode_stream_method,
                                                  image_stream);
        ...
        while ((per_vertex_info = strtok_r(per_vertex_info_iter, "/",
          ...
          switch (per_vertex_info_count) {
            ...
            case 2:
              ...
              [[clang::fallthrough]];
            default:
              LOGE("Format of 'f int/int/int ...' invalid");
              return false;
          }
